...
import {
  Search, Download, Eye, Trash2, Mail, Calendar, Building,
  Filter, X, Upload, Pencil, Plus, Save, FileText, Tag,
} from 'lucide-react';
...
// ===== Data Awal (Dummy) =====
const INITIAL_DATA: SuratMasuk[] = [
  {
    id: '1', nomorSurat: '005/DISDIK/2026', pengirim: 'Dinas Pendidikan Provinsi',
    perihal: 'Undangan Rapat Koordinasi Program Pendidikan', tanggalMasuk: '2026-04-20',
    tanggalSurat: '2026-04-18', tujuan: 'Kepala Dinas Pendidikan Kabupaten',
    sifat: 'Penting', status: 'Baru', keterangan: 'Rapat 25 April 2026',
  },
  {
    id: '2', nomorSurat: '010/SEKDA/2026', pengirim: 'Sekretariat Daerah',
    perihal: 'Pemberitahuan Jadwal Pencairan TPP', tanggalMasuk: '2026-04-22',
    tanggalSurat: '2026-04-21', tujuan: 'Seluruh OPD', sifat: 'Penting',
    status: 'Diproses', keterangan: 'Pencairan minggu pertama Mei 2026',
  },
  {
    id: '3', nomorSurat: '015/SD01/2026', pengirim: 'SD Negeri 1 Lemahabang',
    perihal: 'Laporan Bulanan Kepegawaian', tanggalMasuk: '2026-04-15',
    tanggalSurat: '2026-04-14', tujuan: 'Kabid Dikdas', sifat: 'Biasa',
    status: 'Selesai', keterangan: 'Laporan bulan Maret 2026',
  },
];

// ===== Komponen Badge =====
function SifatBadge({ sifat }: { sifat: SuratMasuk['sifat'] }) {
  const map = {
...
function StatusBadge({ status }: { status: SuratMasuk['status'] }) {
  const map = {
    Baru: 'bg-blue-500 hover:bg-blue-600',
...
// ===== Form Kosong =====
const EMPTY_FORM: Omit<SuratMasuk, 'id'> = {
  nomorSurat: '', pengirim: '', perihal: '', tanggalMasuk: '',
  tanggalSurat: '', tujuan: '', sifat: 'Biasa', status: 'Baru', keterangan: '',
};

export default function SuratMasukPage() {
  // Data state
  const [data, setData] = useState<SuratMasuk[]>(INITIAL_DATA);
  ...
  const openEdit = (surat: SuratMasuk) => {
    setEditId(surat.id);
    setForm({
      nomorSurat: surat.nomorSurat,
      pengirim: surat.pengirim,
      perihal: surat.perihal,
      tanggalMasuk: surat.tanggalMasuk,
  ...
  const handleSave = () => {
    if (!form.nomorSurat || !form.pengirim || !form.perihal || !form.tanggalMasuk) {
      toast.error('Harap isi nomor surat, pengirim, perihal, dan tanggal masuk.');
      return;
    }
  ...
  const totalBaru = data.filter((s) => s.status === 'Baru').length;
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Kelola surat masuk • {totalBaru} surat baru • Total {data.length}
          </p>
        </div>
      ...
      <Card>
                  ...
                  <motion.div key={surat.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }}
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground flex items-center gap-1">
                        <Building className="h-3 w-3" /> {surat.pengirim}
                      </p>
                    </div>
                    <div className="w-28 text-center text-sm text-muted-foreground">
                      {new Date(surat.tanggalMasuk).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric' })}
                    </div>
        ...
        <div className="border-t px-4 py-2 text-xs text-muted-foreground">
          Menampilkan {filteredData.length} dari {data.length} surat masuk
        </div>
      </Card>
      ...
      <Dialog open={showDetail} onOpenChange={setShowDetail}>
        <DialogContent className="max-w-lg">
          <DialogHeader><DialogTitle>Detail Surat Masuk</DialogTitle></DialogHeader>
          {selectedSurat && (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div><p className="text-xs text-muted-foreground">Tanggal Masuk</p><p className="text-sm">{selectedSurat.tanggalMasuk}</p></div>
              </div>
              <Separator />
              <div><p className="text-xs text-muted-foreground">Pengirim</p><p className="text-sm font-medium">{selectedSurat.pengirim}</p></div>
              <div><p className="text-xs text-muted-foreground">Tujuan</p><p className="text-sm">{selectedSurat.tujuan}</p></div>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle>{editId ? 'Edit Surat Masuk' : 'Tambah Surat Masuk'}</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <div>
              <Label htmlFor="nomorSurat">Nomor Surat *</Label>
              <Input id="nomorSurat" value={form.nomorSurat} onChange={(e) => setForm({ ...form, nomorSurat: e.target.value })} placeholder="Contoh: 005/DISDIK/2026" />
            </div>
            <div>
              <Label htmlFor="pengirim">Pengirim *</Label>
              <Input id="pengirim" value={form.pengirim} onChange={(e) => setForm({ ...form, pengirim: e.target.value })} placeholder="Instansi pengirim" />
            </div>
            <div>
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Input id="tanggalMasuk" type="date" value={form.tanggalMasuk} onChange={(e) => setForm({ ...form, tanggalMasuk: e.target.value })} />
              </div>
            </div>
            <div>
              <Label htmlFor="tujuan">Tujuan</Label>
              <Input id="tujuan" value={form.tujuan} onChange={(e) => setForm({ ...form, tujuan: e.target.value })} placeholder="Tujuan surat" />
            </div>
            <div className="grid grid-cols-2 gap-4">
              <div>
                <Label>Sifat</Label>
                <Select value={form.sifat} onValueChange={(val) => setForm({ ...form, sifat: val as SuratMasuk['sifat'] })}>
              ...
              <div>
                <Label>Status</Label>
                <Select value={form.status} onValueChange={(val) => setForm({ ...form, status: val as SuratMasuk['status'] })}>
                  <SelectTrigger><SelectValue /></SelectTrigger>
                  <SelectContent>
