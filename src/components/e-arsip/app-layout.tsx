...
const NAV_ITEMS: NavEntry[] = [
  ...
  { icon: Hourglass, label: 'BUP Pensiun', page: 'bup', adminOnly: true },
  {
    ...
    adminOnly: true,
    children: [
      { icon: Inbox, label: 'Surat Masuk', page: 'surat-masuk' },
      { icon: Send, label: 'Surat Keluar', page: 'surat-keluar' },
      { icon: Archive, label: 'Arsip Surat', page: 'arsip-surat' },
    ],
  },
];
...
function getNotifIcon(type: Notifikasi['type']) {
  switch (type) {
    case 'success': return <Check className="h-3.5 w-3.5 text-emerald-500 shrink-0" />;
...
function SidebarContent({
  ...
  const [expandedGroups, setExpandedGroups] = useState<Set<string>>(() => {
    ...
    for (const entry of filteredItems) {
      if (isNavGroup(entry) && entry.children.some((c) => c.page === activePage)) {
  ...
  const renderNavEntry = (entry: NavEntry) => {
    if (isNavGroup(entry)) {
      ...
      const hasActiveChild = entry.children.some((c) => c.page === activePage);
      ...
      return (
        <div key={entry.label} className="mt-1">
          ...
          >
            ...
            <GroupIcon className={cn('relative z-10 h-[18px] w-[18px] shrink-0 transition-colors', hasActiveChild ? 'text-[#3c6eff]' : 'text-muted-foreground group-hover:text-foreground')} />
            <span className="relative z-10 flex-1 truncate">{entry.label}</span>
            <motion.div animate={{ rotate: isExpanded ? 180 : 0 }} transition={{ duration: 0.2 }} className="relative z-10">
          ...
          <AnimatePresence initial={false}>
            {isExpanded && (
              <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }}
                ...
                <div className="ml-3 mt-0.5 flex flex-col gap-0.5 border-l-2 border-muted pl-3">
                  {entry.children.map((child) => {
                    ...
                    return (
                      <button key={child.page} onClick={() => handleNav(child.page)}
                        className={cn(
                          'group relative flex w-full items-center gap-2.5 rounded-lg px-3 py-2 text-[13px] font-medium transition-all duration-150 outline-none',
                          'focus-visible:ring-2 focus-visible:ring-[#3c6eff]/40 focus-visible:ring-offset-1',
                          isActive ? 'font-semibold text-[#3c6eff]' : 'text-muted-foreground hover:bg-muted/70 hover:text-foreground'
                        )} aria-current={isActive ? 'page' : undefined}>
                        <ChildIcon className={cn('h-4 w-4 shrink-0 transition-colors', isActive ? 'text-[#3c6eff]' : 'text-muted-foreground group-hover:text-foreground')} />
    ...
    return (
      <button key={item.page} onClick={() => handleNav(item.page)}
        ...
        <Icon className={cn('relative z-10 h-[18px] w-[18px] shrink-0 transition-colors', isActive ? 'text-[#3c6eff]' : 'text-muted-foreground group-hover:text-foreground')} />
  ...
  return (
    <div className="flex h-full flex-col bg-white dark:bg-zinc-950">
      <div className="flex shrink-0 items-center gap-3 px-5 py-5">
        <img src="https://i.pinimg.com/736x/76/39/2d/76392d91c9c22d8ec5563b1126cd55b8.jpg" alt="Logo E-Arsip ASN"
      ...
      <ScrollArea className="flex-1 px-3 py-3">
        <nav className="flex flex-col gap-1" role="navigation" aria-label="Menu utama">
          {filteredItems.map((entry) => renderNavEntry(entry))}
        </nav>
      </ScrollArea>
      <Separator />
      <div className="shrink-0 px-4 py-4">
        <div className="flex items-center gap-3">
          <Avatar className="h-9 w-9 border border-muted">
            <AvatarFallback className="bg-[#3c6eff]/10 text-xs font-bold text-[#3c6eff]">
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <Button variant="ghost" size="icon" className="relative h-9 w-9 text-muted-foreground hover:text-foreground" aria-label="Notifikasi">
        ...
        <div className="max-h-72 overflow-y-auto">
          {visibleNotifs.length === 0 ? (
            ...
            visibleNotifs.map((notif) => (
              <button key={notif.id} onClick={() => markNotifRead(notif.id)}
                className={cn(
                ...
                <div className="mt-0.5 rounded-full bg-muted p-1.5">{getNotifIcon(notif.type)}</div>
                <div className="flex min-w-0 flex-1 flex-col gap-0.5">
                  <p className={cn('text-[13px] leading-snug', notif.read ? 'text-muted-foreground' : 'font-medium text-foreground')}>
                    {notif.message}
                  </p>
                  <span className="text-[11px] text-muted-foreground/70">{formatRelativeTime(notif.createdAt)}</span>
                </div>
                {!notif.read && <span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-[#3c6eff]" />}
              </button>
            ))
          )}
        </div>
...
function Topbar({ mobileSidebarOpen, setMobileSidebarOpen }: {
    ...
    <header className="sticky top-0 z-30 flex h-14 shrink-0 items-center gap-3 border-b bg-white/80 px-4 backdrop-blur-md dark:border-zinc-800 dark:bg-zinc-950/80 sm:px-6">
      <Sheet open={mobileSidebarOpen} onOpenChange={setMobileSidebarOpen}>
        <SheetTrigger asChild>
          <Button variant="ghost" size="icon" className="h-9 w-9 text-muted-foreground hover:text-foreground lg:hidden" aria-label="Buka menu">
      ...
      <div className="flex min-w-0 flex-1 items-center">
        <h1 className="truncate text-base font-semibold text-foreground sm:text-lg">{PAGE_TITLES[activePage]}</h1>
      </div>
...
function MobileSidebar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const filteredItems = useMemo(() => {
    ...
    return NAV_ITEMS.map((entry) => {
      if (isNavGroup(entry)) {
        const filteredChildren = entry.children.filter((c) => !c.adminOnly || role === 'admin');
  ...
  return (
    <SidebarContent activePage={activePage} setActivePage={setActivePage} filteredItems={filteredItems}
      currentUser={currentUser} logout={logout} onNavigate={onNavigate} />
  );
}
...
export default function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-muted/30 dark:bg-zinc-900/30">
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <Topbar mobileSidebarOpen={mobileSidebarOpen} setMobileSidebarOpen={setMobileSidebarOpen} />
        <main className="flex-1 overflow-y-auto p-4 sm:p-6">
          <motion.div key={activePage} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
