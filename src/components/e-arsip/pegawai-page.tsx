...
import {
  JENIS_ASN_OPTIONS, JABATAN_OPTIONS, GOLONGAN_OPTIONS,
  KECAMATAN_OPTIONS, UNIT_KERJA_OPTIONS, ALL_JENIS_ASN, JENIS_ASN_BADGE,
} from '@/lib/constants';
import {
  formatDate, getStatusBadgeClass, groupDocsByType,
  downloadTemplateXLS, parseCSVTemplate,
...
export default function PegawaiPage() {
  ...
  const filteredData = useMemo(() => {
    ...
    if (!isPegawaiRole && filterKecamatan) data = data.filter((p) => p.kecamatan === filterKecamatan);
    if (!isPegawaiRole && filterUnitKerja) data = data.filter((p) => p.unitKerja === filterUnitKerja);
    return data;
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (ev) => {
      ...
      rows.forEach((row, idx) => {
        ...
        if (!nip) { errorCount++; messages.push(`Baris ${rowNum}: NIP wajib diisi`); return; }
        if (!nama) { errorCount++; messages.push(`Baris ${rowNum}: Nama wajib diisi`); return; }
        if (jenisASN && !ALL_JENIS_ASN.includes(jenisASN)) { errorCount++; messages.push(`Baris ${rowNum}: Jenis ASN "${jenisASN}" tidak valid`); return; }
        if (pegawaiList.some((p) => p.nip === nip)) { errorCount++; messages.push(`Baris ${rowNum}: NIP "${nip}" sudah terdaftar`); return; }
        addPegawai({ id: Date.now() + idx, nip, nama, jenisASN: jenisASN || '', jabatan, golongan, kecamatan, unitKerja, email, hp, tanggalLahir, status });
        successCount++;
      });
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          {isAdmin && (
            <div className="flex flex-wrap items-center gap-2">
              <Button onClick={handleOpenAdd} className="bg-[#3c6eff] hover:bg-[#3c6eff]/90 text-white gap-2">
      ...
      <Card className="border-border/60 bg-white dark:bg-zinc-950">
        <CardContent className="p-4">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <select value={filterJenisASN} onChange={(e) => updateJenisASN(e.target.value)}
            ...
            <select value={filterStatus} onChange={(e) => updateStatus(e.target.value)}
              className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-xs transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
            ...
            {!isPegawaiRole && (
            <select value={filterKecamatan} onChange={(e) => updateKecamatan(e.target.value)}
              className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-xs transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
            ...
            {!isPegawaiRole && (
            <select value={filterUnitKerja} onChange={(e) => updateUnitKerja(e.target.value)}
              className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 py-1 text-sm shadow-xs transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring">
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-border/50 hover:bg-transparent">
                  ...
                  <TableHead className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground hidden xl:table-cell">Kecamatan</TableHead>
                  ...
                  {isAdmin && (
                    <TableHead className="px-4 py-3 text-xs font-semibold uppercase tracking-wider text-muted-foreground text-right">Aksi</TableHead>
                  )}
                </TableRow>
              </TableHeader>
              <TableBody>
                {paginatedData.length === 0 ? (
                  <TableRow>
                    <TableCell colSpan={isAdmin ? 10 : 9} className="px-4 py-12 text-center">
                  ...
                  paginatedData.map((pg, idx) => {
                    ...
                    return (
                      <TableRow key={pg.id} className="border-border/40 transition-colors hover:bg-muted/40">
                        ...
                        <TableCell className="px-4 py-3 text-sm text-muted-foreground hidden xl:table-cell">{pg.kecamatan || '-'}</TableCell>
          ...
          {filteredData.length > ITEMS_PER_PAGE && (
            <div className="flex items-center justify-between border-t px-4 py-3">
              <p className="text-sm text-muted-foreground">
                Menampilkan <span className="font-medium text-foreground">{(currentPage - 1) * ITEMS_PER_PAGE + 1}</span> - <span className="font-medium text-foreground">{Math.min(currentPage * ITEMS_PER_PAGE, filteredData.length)}</span> dari <span className="font-medium text-foreground">{filteredData.length}</span> pegawai
              </p>
        ...
        <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-lg font-semibold">
              <Users className="h-5 w-5 text-[#3c6eff]" /> Detail Pegawai
            </DialogTitle>
          </DialogHeader>
          {detailPegawai && (
            <div className="space-y-6">
      ...
      <Dialog open={showUploadModal} onOpenChange={(v) => !v && setShowUploadModal(false)}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-lg font-semibold">
              <Upload className="h-5 w-5 text-amber-600" /> Upload Template Pegawai
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-4 py-2">
            {!importResult ? (
              <>
                <div className="space-y-2">
                  <p className="text-sm text-muted-foreground">Upload file CSV sesuai template yang telah diunduh. Format kolom:</p>
                ...
                <div className="flex flex-col items-center gap-3 rounded-lg border-2 border-dashed border-muted-foreground/30 p-8 transition-colors hover:border-muted-foreground/50">
                  ...
                  <p className="text-sm text-muted-foreground">Drag & drop atau klik untuk memilih file</p>
                  <p className="text-xs text-muted-foreground/70">Format: CSV (comma-separated values)</p>
                ...
                {importResult.messages.length > 0 && (
                  <div className="max-h-48 overflow-y-auto space-y-1.5">
                    ...
                    {importResult.messages.map((msg, idx) => (
                      <div key={idx} className="flex items-start gap-2 rounded-md bg-red-50 px-3 py-2 text-xs text-red-700 dark:bg-red-950/20 dark:text-red-400">
                        <X className="mt-0.5 h-3.5 w-3.5 shrink-0" /><span>{msg}</span>
                      </div>
                    ))}
                  </div>
                )}
...
function DetailItem({ label, value, mono, bold, badge, badgeClass }: {
  label: string; value: string; mono?: boolean; bold?: boolean; badge?: boolean; badgeClass?: string;
}) {
  return (
