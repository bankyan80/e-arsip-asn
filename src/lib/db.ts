...
// Helper: safe map row to Pegawai
...
// Helper: safe map row to Dokumen
...
// Helper: safe map row to Notifikasi
...
export async function fetchPegawai(): Promise<Pegawai[]> {
  try {
    ...
    if (error) {
      console.error('[DB] Error fetching pegawai:', error.message, 'code:', error.code);
      return [];
    }

    return (data || []).map((row) => rowToPegawai(row as Record<string, unknown>));
  } catch (err) {
    console.error('[DB] EXCEPTION fetching pegawai:', err);
    return [];
  }
}
...
export async function fetchPegawaiByNIP(nip: string): Promise<Pegawai | undefined> {
  try {
    ...
    console.error('[DB] EXCEPTION fetching pegawai by NIP:', err);
    return undefined;
  }
}
...
export async function addPegawaiToDB(pg: Pegawai): Promise<Pegawai | null> {
  try {
    ...
    if (error) {
      console.error('[DB] Error adding pegawai:', error.message, 'code:', error.code);
      return null;
    }

    return rowToPegawai(data as Record<string, unknown>);
  } catch (err) {
    console.error('[DB] EXCEPTION adding pegawai:', err);
    return null;
  }
}
...
export async function updatePegawaiInDB(id: number, pg: Partial<Pegawai>): Promise<boolean> {
  try {
    ...
    if (error) {
      console.error('[DB] Error updating pegawai:', error.message);
      return false;
    }
    return true;
  } catch (err) {
    console.error('[DB] EXCEPTION updating pegawai:', err);
    return false;
  }
}
...
export async function deletePegawaiFromDB(id: number): Promise<boolean> {
  try {
    ...
    if (error) {
      console.error('[DB] Error deleting pegawai:', error.message);
      return false;
    }
    return true;
  } catch (err) {
    console.error('[DB] EXCEPTION deleting pegawai:', err);
    return false;
  }
}
...
export async function fetchDokumen(): Promise<Dokumen[]> {
  try {
    ...
    if (error) {
      console.error('[DB] Error fetching dokumen:', error.message, 'code:', error.code);
      return [];
    }

    return (data || []).map((row) => rowToDokumen(row as Record<string, unknown>));
  } catch (err) {
    console.error('[DB] EXCEPTION fetching dokumen:', err);
    return [];
  }
}
...
export async function fetchDokumenByPegawaiId(pegawaiId: number): Promise<Dokumen[]> {
  try {
    ...
    if (error) {
      console.error('[DB] Error fetching dokumen for pegawai:', error.message);
      return [];
    }

    return (data || []).map((row) => rowToDokumen(row as Record<string, unknown>));
  } catch (err) {
    console.error('[DB] EXCEPTION fetching dokumen for pegawai:', err);
    return [];
  }
}
...
export async function uploadFileAndGetUrl(file: File, pegawaiId: number): Promise<{ url: string; filePath: string } | null> {
  try {
    ...
    console.error('[DB] EXCEPTION uploading file:', err);
    return null;
  }
}
...
export async function addDokumenToDB(doc: Omit<Dokumen, 'id'>): Promise<Dokumen | null> {
  try {
    ...
    console.error('[DB] EXCEPTION adding dokumen:', err);
    return null;
  }
}
...
export async function updateDokumenStatusInDB(id: number, status: 'Pending' | 'Approved' | 'Rejected'): Promise<boolean> {
  try {
    ...
    if (error) {
      console.error('[DB] Error updating dokumen status:', error.message);
      return false;
    }
    return true;
  } catch (err) {
    console.error('[DB] EXCEPTION updating dokumen status:', err);
    return false;
  }
}
...
export async function updateDokumenKeteranganInDB(id: number, keterangan: string): Promise<boolean> {
  try {
    ...
    if (error) {
      console.error('[DB] Error updating dokumen keterangan:', error.message);
      return false;
    }
    return true;
  } catch (err) {
    console.error('[DB] EXCEPTION updating dokumen keterangan:', err);
    return false;
  }
}
...
export async function deleteDokumenFromDB(id: number, filePath?: string): Promise<boolean> {
  try {
    ...
    if (error) {
      console.error('[DB] Error deleting dokumen:', error.message);
      return false;
    }
    return true;
  } catch (err) {
    console.error('[DB] EXCEPTION deleting dokumen:', err);
    return false;
  }
}
...
export async function fetchNotifikasi(): Promise<Notifikasi[]> {
  try {
    ...
    if (error) {
      console.error('[DB] Error fetching notifikasi:', error.message, 'code:', error.code);
      return [];
    }

    return (data || []).map((row) => rowToNotifikasi(row as Record<string, unknown>));
  } catch (err) {
    console.error('[DB] EXCEPTION fetching notifikasi:', err);
    return [];
  }
}
...
export async function addNotifikasiToDB(message: string, type: Notifikasi['type']): Promise<Notifikasi | null> {
  try {
    ...
    if (error) {
      console.error('[DB] Error adding notifikasi:', error.message, 'code:', error.code);
      return null;
    }

    return rowToNotifikasi(data as Record<string, unknown>);
  } catch (err) {
    console.error('[DB] EXCEPTION adding notifikasi:', err);
    return null;
  }
}
...
export async function markNotifikasiReadInDB(id: number): Promise<boolean> {
  try {
    ...
    if (error) {
      console.error('[DB] Error marking notifikasi read:', error.message);
      return false;
    }
    return true;
  } catch (err) {
    console.error('[DB] EXCEPTION marking notifikasi read:', err);
    return false;
  }
}
...
export async function markAllNotifikasiReadInDB(): Promise<boolean> {
  try {
    ...
    if (error) {
      console.error('[DB] Error marking all notifikasi read:', error.message);
      return false;
    }
    return true;
  } catch (err) {
    console.error('[DB] EXCEPTION marking all notifikasi read:', err);
    return false;
  }
}
...
export async function clearNotifikasiInDB(): Promise<boolean> {
  try {
    ...
    if (error) {
      console.error('[DB] Error clearing notifikasi:', error.message);
      return false;
    }
    return true;
  } catch (err) {
    console.error('[DB] EXCEPTION clearing notifikasi:', err);
    return false;
  }
}