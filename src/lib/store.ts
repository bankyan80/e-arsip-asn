...
export const useArsipStore = create<ArsipStore>()((set, get) => ({
  ...
  isLoading: false,
  addPegawai: async (p) => { const result = await addPegawaiToDB(p); if (result) set((s) => ({ pegawaiList: [...s.pegawaiList, result] })); },
  updatePegawai: async (id, data) => { const success = await updatePegawaiInDB(id, data); if (success) set((s) => ({ pegawaiList: s.pegawaiList.map((p) => (p.id === id ? { ...p, ...data } : p)) })); },
  deletePegawai: async (id) => { const success = await deletePegawaiFromDB(id); if (success) set((s) => ({ pegawaiList: s.pegawaiList.filter((p) => p.id !== id), dokumenList: s.dokumenList.filter((d) => d.pegawaiId !== id) })); },
  addDokumen: async (d) => { const result = await addDokumenToDB(d); if (result) set((s) => ({ dokumenList: [...s.dokumenList, result] })); },
  addDokumenWithFile: async (file, d) => {
    ...
    if (result) set((s) => ({ dokumenList: [...s.dokumenList, result] }));
    else { const { supabase } = await import('./supabase'); supabase.storage.from('dokumen').remove([uploadResult.filePath]); throw new Error('Gagal menyimpan data dokumen'); }
  },
  updateDokumenStatus: async (id, status) => { const success = await updateDokumenStatusInDB(id, status); if (success) set((s) => ({ dokumenList: s.dokumenList.map((d) => (d.id === id ? { ...d, status } : d)) })); },
  updateDokumenKeterangan: async (id, keterangan) => { const success = await updateDokumenKeteranganInDB(id, keterangan); if (success) set((s) => ({ dokumenList: s.dokumenList.map((d) => (d.id === id ? { ...d, keterangan } : d)) })); },
  deleteDokumen: async (id) => { const doc = get().dokumenList.find((d) => d.id === id); const success = await deleteDokumenFromDB(id, doc?.filePath); if (success) set((s) => ({ dokumenList: s.dokumenList.filter((d) => d.id !== id) })); },
  addNotifikasi: async (message, type) => { const result = await addNotifikasiToDB(message, type); if (result) set((s) => ({ notifikasiList: [result, ...s.notifikasiList].slice(0, 50) })); },
  markNotifRead: async (id) => { const success = await markNotifikasiReadInDB(id); if (success) set((s) => ({ notifikasiList: s.notifikasiList.map((n) => (n.id === id ? { ...n, read: true } : n)) })); },
  markAllNotifRead: async () => { const success = await markAllNotifikasiReadInDB(); if (success) set((s) => ({ notifikasiList: s.notifikasiList.map((n) => ({ ...n, read: true })) })); },
  clearNotifikasi: async () => { const success = await clearNotifikasiInDB(); if (success) set({ notifikasiList: [] }); },
...
export function restoreAuth() {
  ...
  if (user) { useArsipStore.setState({ currentUser: user, isLoggedIn: true }); return true; }
  return false;
}